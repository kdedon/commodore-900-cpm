...
--console=serial|probe writes a `console' line under the CP/M entry.  kboot
decides the console it hands over (bi_console) and CP/M obeys it: `serial'
pins the serial line, `probe' has kboot test the framebuffers.  Without the
option no line is written, which kboot reads as `probe'.  A kboot older than
the keyword ignores the line.
...
def kbootcfg(base, cpmb_base, bootinfo=True, console=None):
    ...
    `console', when given, is the entry's `console' line; None writes none,
    which kboot reads as `probe'.
    """
    con = [] if console is None else [
        "# console serial|probe: the console kboot hands CP/M.",
        "console %s" % console,
    ]
    if not bootinfo:
...
def main():
    ...
    for a in sys.argv[1:]:
        if a.startswith('--kboot='):
            ...
            # Only the two values kboot knows: it refuses the entry for any
            # other, and there is deliberately no value that forces video.
            console = a.split('=', 1)[1]
            if console not in ('serial', 'probe'):
                sys.exit("mkcpmdisk: --console must be serial or probe")
    ...
    cfg = kbootcfg(base, cpmb_base, bootinfo, console)
    if mutate == 'cfg':
