...
char	ro_msg[]	= "Read Only (RO)";	/* was "readonly": cc0-z8001 */
						/*   reserves that as a type */
						/*   qualifier keyword       */
						/* second deviation from DRI */
						/*   source: columns(), below,*/
						/*   drops the #ifdef HILO   */
						/*   byte-order trick -- see */
						/*   the comment there       */
...
{						/*   console (gives default */
	...
	if (HAS_GSET_SCB(ver))
		/* Deviation from DRI source (see also ro_msg above): DRI's   */
		/* #ifdef HILO ">> 8" (and the LOHI "no shift" else-branch)   */
		/* assumed the SCB word's two bytes -- @CONWIDTH at 0x1a, the */
		/* console width, and @COLUMN at 0x1b, the cursor's current   */
		/* column -- swap places depending on the target CPU's       */
		/* native byte order.  This port's BDOS (src/bdos/scb.c,      */
		/* scbword()) always builds the word low byte first, matching*/
		/* DRI's own 8080 reference (BDOS30.ASM func49) regardless of */
		/* host CPU, so neither of DRI's branches is right here: HILO's*/
		/* ">> 8" reads back @COLUMN instead of the width, and LOHI's */
		/* unshifted word conflates both bytes into one number.  The  */
		/* width is always the low byte, on every target this BDOS    */
		/* serves.                                                    */
		return (_gset_scb(&colpb) & 0xff);
	else
		return (WIDTH);
}
...
{						/* Return TRUE if allocation*/
	...
	{					/*   bits long		    */
		...
		/* THE PARENTHESES ARE OURS, AND THEY MATTER.  DRI wrote
		   this as
			block = ((UWORD) second << 8) +
				 (UWORD) first & 0xff;
		   which is not what it looks like: `+' binds tighter than
		   `&', so the mask lands on the SUM and every 16-bit block
		   number comes back truncated to eight bits.  Block 256
		   then reads as block 0, and allocate() reports every file
		   past the first 1 MB of a disk as a "Space Allocation
		   Conflict" with a file at the front of it.  Verbatim in
		   the cpm8k13 drop's STAT.C, so it is DRI's and not this
		   port's -- and latent for forty years, because it can
		   only fire on a drive with 16-bit block numbers holding
		   more than 256 of them, which no 1983 floppy was.  Drive
		   A: here is 10 MB.  It crossed 256 blocks when src/app's
		   programs and their source were staged onto it, and
		   `make verify' -- which runs STAT over that drive -- is
		   what caught it.  */
