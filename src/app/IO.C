...
/* db_nerror() is declared once, here, matching its definition at the
   foot of this file: ZCC1 hoists a block-scope function declaration to
   file scope, so the three spellings this file used to carry -- struct
   relation *, struct scan * and the int * it really is -- collided.
   The call sites cast instead.  (c900) */
int *db_nerror();
...
#else
#ifdef CPM68K
    if ((fd = openb(filename,0)) == -1)
#else
    if ((fd = open(filename,0)) == -1)
#endif
#endif
        return ((struct relation *) db_nerror(RELFNF));

    /* allocate a new relation structure */
    if ((rptr = malloc(sizeof(struct relation))) == NULL) {
        close(fd);
        return ((struct relation *) db_nerror(INSMEM));
    }
  ...
  char *rname;
{
    ...
    if ((sptr = malloc(sizeof(struct scan))) == NULL)
        return ((struct scan *) db_nerror(INSMEM));
    ...
    if (rptr->rl_scnref++ == 0) {
...
#else
#ifdef CPM68K
        if ((rptr->rl_fd = openb(filename,2)) == -1) {
#else
        if ((rptr->rl_fd = open(filename,2)) == -1) {
            ...
            return ((struct scan *) db_nerror(RELFNF));
        }
    }
...
int db_cvword(bytes)
  char bytes[2];
{
    return (((bytes[0] & 0377) << 8) + (bytes[1] & 0377));
}
