...
/* PORT NOTE.  This file's two private operand readers are spelled
   comgnum() and comgstr(), not get_number() and get_string(): ZCC gives
   a `static' function EXTERNAL linkage -- MTH.O's symbol table carries
   _getsign, which MTH.C declares static -- so they collided at LD8K with
   SCN.C's own private get_number() and get_string().  Nothing else here
   is changed from the cpm68k_pgms copy.  (c900) */
...
LOCAL union codecell code[CODEMAX+1];
LOCAL int cndx;
