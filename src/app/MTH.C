...
/* PORT NOTE.  This file's private compare() is spelled mthcomp(): ZCC
   gives a `static' function external linkage, so it collided at LD8K
   with SRT.C's own private compare().  Nothing else is changed.  (c900) */
  ...
  char *arg1,*arg2;
{
    ...
    if (sign1 == sign2) {
        if (sign1 == 1)
            return (mthcomp(arg1,arg2));
        else
            return (mthcomp(arg2,arg1));
    }
