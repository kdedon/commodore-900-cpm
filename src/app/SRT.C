...
/* PORT NOTE.  Two private functions are renamed here for the same reason
   -- ZCC gives a `static' function external linkage, so an obvious name
   kept privately by two files collides at LD8K.  compare() is srtcomp()
   (against MTH.C's) and find_attr() is srtfatt() (against SEL.C's).
   Nothing else is changed from the cpm68k_pgms copy.  (c900) */
  ...
  struct scan *sptr;
{
    ...
    while (TRUE) {
        ...
        if (!srtfatt(sptr,newskey,dbv_tstring)) {
            free(newskey);
            return (NULL);
        }
...
static int srtfatt(sptr,newskey,aname)
...
static int srtcomp(skeys,sptr1,sptr2)
