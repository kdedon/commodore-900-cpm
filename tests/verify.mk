...
# ---- CP/M-86 shim, host side (src/cmd/i86*.c) ----
...
# tests/i86corpus/ holds four of DRI's own CP/M-86 programs (PIP, ED, GENCMD,
# SUBMIT, from the cpm86pc drop, under the 9 July 2022 DRDOS, Inc. grant):
# real headers, and four real prologues that execute to their first BDOS
# call.  They are TEST INPUT ONLY -- nothing stages them onto a disk image,
...
# writes the one fixture that is not a .CMD: I86HEX.H86, GENCMD's input,
# real DRI hex with real checksums around a hand-assembled 8086 program.
I86SRC = src/cmd/i86dec.c src/cmd/i86exec.c src/cmd/i86load.c src/cmd/i86bdos.c
I86CORPUS = tests/i86corpus
I86FIX = build/cmdfix
.PHONY: i86test
build/i86test: tests/i86test.c $(I86SRC) src/cmd/i86.h | $(OBJDIR)
	$(HOSTCC) -std=gnu89 -w -DHOSTCC -o $@ tests/i86test.c $(I86SRC)
...
# ---- CP/M-80 shim, host side (src/cmd/z80*.c) ----
...
#
# tests/z80corpus/ holds four of DRI's own CP/M 3 programs (PIP, DUMP,
...
# is here to answer.
Z80SRC = src/cmd/z80dec.c src/cmd/z80exec.c src/cmd/z80load.c \
	 src/cmd/z80bdos.c
Z80CORPUS = tests/z80corpus
.PHONY: z80test
build/z80test: tests/z80test.c $(Z80SRC) src/cmd/z80.h | $(OBJDIR)
	$(HOSTCC) -std=gnu89 -w -DHOSTCC -o $@ tests/z80test.c $(Z80SRC)
...
# 25 exists.  Z80.Z8K (src/cmd/z80.c) asks the BIOS for a 64 KB segment,
...
# instruction.  VERIFY.IN is the 1,024 bytes tests/z80test.c t_pip()
...
# stub in tests/z80test.c had BDOS function 44, set multi-sector count,
...
# is test input (tests/z80corpus/SOURCES), VERIFY.IN is generated, and
...
verify-z80pip: all
	...
	@grep -q 'z80: 13024 instructions, 29 BDOS calls, 1831 flag' $(Z80PLOG) \
		|| { echo "verify-z80pip: FAIL -- the target run did not take the same"; \
		     ...
		     echo "            materialisations -- tests/z80test.c"; \
...
# .CMD is TWO groups, code and data, so CPM86.Z8K (src/cmd/i86.c) asks
...
#
# WHY PIP LEADS.  tests/i86corpus/ holds PIP, ED, GENCMD and SUBMIT, and
...
# exactly the reason tests/i86corpus/SOURCES gives for PIP being in the
...
# pair (35 and 36) whose field order src/cmd/i86bdos.c ranswap() had to
...
# lane.  I86IN.TXT is the 1024 bytes tests/i86test.c section 8b copies --
...
# is test input (see tests/i86corpus/SOURCES), I86IN.TXT is generated,
...
# src/cmd/i86load.c galloc(), and its input is a file rather than a
...
verify-i86: all build/i86sub-host.bin build/i86hex-host.cmd
	...
	@# IT.  Against the G-Min floor src/cmd/i86load.c galloc() used to
	...
	@# p_multi()).  It is the same file tests/i86test.c loads from
	...
	@grep -q 'i86: 2689 instructions, 9 BDOS calls' $(I86LOG) \
		|| { echo "verify-i86: FAIL -- the target run of SUBMIT did not"; \
		     ...
		     echo "            instructions, 9 BDOS calls -- tests/i86test.c"; \
	...
	@# tests/i86test.c section 8c writes as it runs, and not against a
	...
	@# tests/i86test.c section 8d LOADS AND RUNS the .CMD and reads
...
# and CONN.Z8K (src/cmd/conn.c) reads the count off the running machine
...
# (tests/z80test.c gmem[0x10000], tests/i86test.c bseg/cseg[65536]), so
# with -fsanitize=address a write one byte past it ABORTS the run and the
# abort is the assertion.  It is what caught src/cmd/z80load.c's read of
# byte 256 of a 256-byte RSX image, which no return code reports at all.
#
# `timeout' is here for the third defect: src/cmd/i86dec.c's prefix loop
...
# (tests/i86test.c t_prefix() carries its own alarm too, so the failure
# is named rather than merely timed out.)
build/z80test-asan: tests/z80test.c $(Z80SRC) src/cmd/z80.h | $(OBJDIR)
	$(HOSTCC) -std=gnu89 -w -DHOSTCC -fsanitize=address -g -o $@ \
		tests/z80test.c $(Z80SRC)
build/i86test-asan: tests/i86test.c $(I86SRC) src/cmd/i86.h | $(OBJDIR)
	$(HOSTCC) -std=gnu89 -w -DHOSTCC -fsanitize=address -g -o $@ \
		tests/i86test.c $(I86SRC)
	...
	@timeout 300 ./build/i86test-asan $(I86CORPUS) $(I86FIX) > build/verify-shim-i86.log 2>&1; \
	 ...
	 test $$rc = 0 \
		|| { echo "verify-shim: FAIL -- the CP/M-86 shim (rc $$rc; 124 means it"; \
		     ...
		     exit 1; }
	@grep -q 'z80test: 814 checks, 0 failures' build/verify-shim-z80.log \
		|| { echo "verify-shim: FAIL -- the CP/M-80 suite did not run all 814 of its"; \
		     echo "             checks (`grep -o '[0-9]* checks, [0-9]* failures' build/verify-shim-z80.log`)."; \
		     echo "             A smaller passing run is not a pass."; exit 1; }
	@grep -q 'i86test: 1475 checks, 0 failures' build/verify-shim-i86.log \
		|| { echo "verify-shim: FAIL -- the CP/M-86 suite did not run all 1475 of its"; \
